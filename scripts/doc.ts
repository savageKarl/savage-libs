...
import { generateFiles, readJsonFile, writeJsonFile } from 'savage-node'
...
async function generateSidebarConfig() {
  const paths = resolvedPkgNames.map((pkgName) =>
    normalizePath(`${docsPath}/packages/${pkgName}/sidebar.json`)
  )

  const contents = paths.map((p) => readFileSync(p, { encoding: 'utf-8' }))
  ...
  generateFiles({ [sidebarFilePath]: stringify(contentRecord) })

  if (mode !== 'dev') return
  chokidar.watch(paths).on('change', (p) => {
    const fullSideBar = readJsonFile(sidebarFilePath)
    const currentSidebar = readJsonFile(p)

    writeJsonFile(sidebarFilePath, Object.assign(fullSideBar, currentSidebar))
  })
}
...
async function generateRewrites() {
  ...
  watcher.on('unlink', (path) => {
    if (isNotMarkdownFile(path)) return
